...
- Custom Vencord Manager is now **v0.1.6**. v0.1.5 introduced the approved React/Tailwind + WebView2 dashboard, ReUI-inspired inverted-black visuals, matching native fallback/dialog polish, and fixed 1040x800 window; v0.1.6 removes the normal-startup legacy WPF flash by warming WebView2 earlier and revealing React only after native state is rendered. The manager backend and per-client isolation semantics remain unchanged.
